...
import MetricsGraph, {ContextMenuItemOrSubmenu, SeriesPoint, Series} from '../MetricsGraph';
...
const ProfileMetricsGraph = ({
  ...
  const selectedPoint = useMemo((): SeriesPoint | null => {
    if (profile !== null && (profile instanceof MergedProfileSelection)) {
      ...
      if (seriesIndex !== undefined && seriesIndex !== -1 && originalSeries !== undefined && originalSeries[seriesIndex] != null) {
        const series = originalSeries[seriesIndex];
        const pointIndex = series.samples.findIndex((sample) => {
          return sample.timestamp?.seconds === BigInt(profile.mergeFrom / 1_000_000_000n) &&
  ...
  const {sampleUnit, sampleType, yAxisLabel, yAxisUnit} = useMemo(() => {
    ...
    const isDeltaType = profile !== null ? (profile as MergedProfileSelection)?.query.profType.delta : false;
          ...
          <MetricsGraph
            ...
            onSampleClick={(closestPoint: SeriesPoint): void => {
              ...
              if (originalSeries?.[closestPoint.seriesIndex] != null) {
                ...
                if ((originalPoint.timestamp != null) && (originalPoint.valuePerSecond !== undefined)) {
                  const timestampNanos = (originalPoint.timestamp.seconds * 1_000_000_000n) + BigInt(originalPoint.timestamp.nanos);
                  onPointClick(
            ...
            renderTooltipContent={(seriesIndex: number, pointIndex: number) => {
              if (originalSeries?.[seriesIndex]?.samples?.[pointIndex] != null) {
                ...
                if (originalPoint.timestamp != null && (originalPoint.valuePerSecond !== undefined)) {
                  const timestampMs = (Number(originalPoint.timestamp.seconds) * 1000) + (originalPoint.timestamp.nanos / 1_000_000);
                        ...
                        <span className="my-2 block text-gray-700 dark:text-gray-300">
                          <table className="table-auto">
                            <tbody>
                              ...
                              {originalPoint.duration != null && Number(originalPoint.duration) > 0 && (
                                <tr>
                                  <td className="w-1/4">Duration</td>
                                  <td className="w-3/4">
                                    {valueFormatter(Number(originalPoint.duration.toString()), 'nanoseconds', 2)}
                                  </td>
                                </tr>
                              )}
                              <tr>
