...
func (c *cachedLiner) newExternalExecutableLiner(
	...
	addr2lineCmd := exec.Command(c.externalAddr2linePath, "--exe", filepath, "-fC")
	...
	scanner := bufio.NewReader(addr2lineOutput)
...
func (l *externalExecutableLiner) PCToLines(ctx context.Context, pc uint64) ([]profile.LocationLine, error) {
	...
	line1, err := l.scanner.ReadString('\n')
	if err != nil {
		return nil, fmt.Errorf("error scanning addr2line output: %w\n", err)
	}
	line2, err := l.scanner.ReadString('\n')
	if err != nil {
		return nil, fmt.Errorf("error scanning addr2line output: %w\n", err)
	}
