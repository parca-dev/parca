...
func (s *Symbolizer) symbolizeLocationsForMapping(ctx context.Context, m *pb.Mapping, locations []*pb.Location) ([][]profile.LocationLine, liner, error) {
	...
	if !found {
		...
		liner, err = s.newLiner(f.Name(), e, m.BaseAddress, m.IsRawAddresses, dbginfo.Quality)
		if err != nil {
...
func (s *Symbolizer) newLiner(filepath string, f *elf.File, base uint64, isRawAddresses bool, quality *debuginfopb.DebuginfoQuality) (liner, error) {
	switch {
	case quality.HasDwarf:
		lnr, err := addr2line.DWARF(s.logger, filepath, f, base, isRawAddresses, s.demangler)
		if err != nil {
	...
	case quality.HasGoPclntab:
		lnr, err := addr2line.Go(s.logger, filepath, f, base, isRawAddresses)
		if err != nil {
	...
	case quality.HasSymtab || quality.HasDynsym:
		lnr, err := addr2line.Symbols(s.logger, filepath, f, base, isRawAddresses, s.demangler)
		if err != nil {
