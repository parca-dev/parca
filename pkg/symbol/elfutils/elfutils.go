...
func (f *File) HasDWARF() (bool, error) {
	sections, err := readableDWARFSections(f.elf)
	if err != nil {
...
func (f *File) IsSymbolizableGoObjFile() (bool, error) {
	...
	if sec := f.elf.Section(".gopclntab"); sec != nil {
...
func (f *File) HasSymbols() (bool, error) {
	for _, section := range f.elf.Sections {
...
// Validate returns an error if the given object file is not valid.
func (f *File) Validate() error {
	// TODO(kakkoyun): How can we improve this without allocating too much memory.
	if len(f.elf.Sections) == 0 {
