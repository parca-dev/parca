...
func TestFlamegraphTrimmingAndFiltering(t *testing.T) {
	...
	tracer := noop.NewTracerProvider().Tracer("")
	p.Samples, filtered, err = FilterProfileData(ctx, tracer, mem, p.Samples, "b", map[string]struct{}{}, map[string]struct{}{}) // querying for "b" should filter out the "5.c" function.
	require.NoError(t, err)
	defer func() {
