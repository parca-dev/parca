...
func (m *FrameFilter_FilenameFrameFilter) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *FrameFilter_FilenameFrameFilter) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	i := len(dAtA)
	if m.FilenameFrameFilter != nil {
		size, err := m.FilenameFrameFilter.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *FilenameFrameFilter) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *FilenameFrameFilter) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *FilenameFrameFilter) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.IncludeFilenames) > 0 {
		for iNdEx := len(m.IncludeFilenames) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.IncludeFilenames[iNdEx])
			copy(dAtA[i:], m.IncludeFilenames[iNdEx])
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.IncludeFilenames[iNdEx])))
			i--
			dAtA[i] = 0xa
		}
	}
...
func (m *FrameFilter_FilenameFrameFilter) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	if m.FilenameFrameFilter != nil {
		l = m.FilenameFrameFilter.SizeVT()
		n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
	}
	return n
}
...
func (m *FilenameFrameFilter) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	if len(m.IncludeFilenames) > 0 {
		for _, s := range m.IncludeFilenames {
			l = len(s)
			n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
		}
	}
...
func (m *FrameFilter) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field FilenameFrameFilter", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if oneof, ok := m.Filter.(*FrameFilter_FilenameFrameFilter); ok {
				if err := oneof.FilenameFrameFilter.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
					return err
				}
			} else {
				v := &FilenameFrameFilter{}
				if err := v.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
					return err
				}
				m.Filter = &FrameFilter_FilenameFrameFilter{FilenameFrameFilter: v}
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
...
func (m *FilenameFrameFilter) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: FilenameFrameFilter: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: FilenameFrameFilter: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field IncludeFilenames", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.IncludeFilenames = append(m.IncludeFilenames, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
