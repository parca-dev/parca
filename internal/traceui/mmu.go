...
func getMMUCurve(r *http.Request, events []*trace.Event) ([][]trace.MutatorUtil, *trace.MMUCurve, error) {
...
func (s *Server) HTTPMMU(w http.ResponseWriter, r *http.Request) {
	type Templ struct {
		Path string
	}

	if err := templMMU.Execute(w, Templ{
		Path: s.path,
	}); err != nil {
...
func (s *Server) HTTPMMUPlot(w http.ResponseWriter, r *http.Request) {
	mu, mmuCurve, err := getMMUCurve(r, s.events)
	if err != nil {
...
var templMMU = template.Must(template.New("").Parse(`
<!doctype html>
<html>
  <head>
    <meta charset="utf-8">
    ...
    <script type="text/javascript">
      ...
      function refreshChart() {
        ...
        $.getJSON('{{.Path}}/mmuPlot?flags=' + mmuFlags())
      ...
      function selectHandler() {
        ...
        var url = '{{.Path}}/mmuDetails?window=' + windowNS + '&flags=' + mmuFlags();
        $.getJSON(url)
         .fail(function(xhr, status, error) {
...
func (s *Server) HTTPMMUDetails(w http.ResponseWriter, r *http.Request) {
	_, mmuCurve, err := getMMUCurve(r, s.events)
	if err != nil {
		http.Error(w, fmt.Sprintf("failed to parse events: %v", err), http.StatusInternalServerError)
		return
	}
	...
	for _, ui := range worst {
		links = append(links, newLinkedUtilWindow(ui, time.Duration(window), s.ranges))
	}
...
func newLinkedUtilWindow(ui trace.UtilWindow, window time.Duration, ranges []Range) linkedUtilWindow {
