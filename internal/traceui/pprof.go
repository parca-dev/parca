...
func (s *Server) PprofByGoroutine(compute func(io.Writer, map[uint64][]interval, []*trace.Event) error) func(w io.Writer, r *http.Request) error {
	return func(w io.Writer, r *http.Request) error {
		id := r.FormValue("id")
		events := s.events
		gToIntervals, err := s.PprofMatchingGoroutines(id, events)
		if err != nil {
			return err
		}
...
func (s *Server) PprofByRegion(compute func(io.Writer, map[uint64][]interval, []*trace.Event) error) func(w io.Writer, r *http.Request) error {
	return func(w io.Writer, r *http.Request) error {
		filter, err := newRegionFilter(r)
		if err != nil {
			return err
		}
		gToIntervals, err := s.PprofMatchingRegions(filter)
		if err != nil {
			return err
		}
		events := s.events
...
func (s *Server) PprofMatchingGoroutines(id string, events []*trace.Event) (map[uint64][]interval, error) {
	if id == "" {
		return nil, nil
	}
	...
	for _, g := range gs {
		...
		endTime := g.EndTime
		if g.EndTime == 0 {
			endTime = lastTimestamp(s.t) // the trace doesn't include the goroutine end event. Use the trace end time.
		}
...
func (s *Server) PprofMatchingRegions(filter *regionFilter) (map[uint64][]interval, error) {
