...
func splitTrace(res trace.ParseResult) ([]Range, error) {
	var ranges []Range
	...
	s, c := splittingTraceConsumer(100<<20, ranges) // 100M
	if err := generateTrace(params, c); err != nil {
		return nil, err
	}
	return s.Ranges, nil
...
func splittingTraceConsumer(max int, ranges []Range) (*splitter, traceConsumer) {
	type eventSz struct {
		Time float64
		Sz   int
	}
